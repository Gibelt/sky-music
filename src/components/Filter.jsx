...
export default function Filter() {
    ...
    return (
      <div className={s.filter}>
        ...
        >
          {performerVisible && (
            <FilterList list={performers} mode="normal" class={`${s.list} ${s.performers}`} />
          )}
          исполнителю
        </div>
        <div
        ...
        >
          {yearVisible && <FilterList class={`${s.list} ${s.year}`} mode="radio" />}
          году выпуска
        </div>
        <div
          role="presentation"
        ...
        >
          {genreVisible && <FilterList list={genres} mode="normal" class={`${s.list} ${s.genre}`} />}
          жанру
        </div>
      </div>
    )
